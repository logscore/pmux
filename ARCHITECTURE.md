...
No dnsmasq, no Caddy, no Nginx. Everything is built into the `pmux` binary.

## Commands

| Command | Description |
|---------|-------------|
| `pmux run "<command>"` | Run command with auto port/domain/proxy |
| `pmux list` | List active tunnels |
| `pmux stop <domain>` | Stop a specific tunnel |
| `pmux proxy start` | Start proxy as background daemon |
| `pmux proxy run` | Run proxy in foreground (debugging) |
| `pmux proxy stop` | Stop the proxy daemon |
| `pmux teardown` | Stop all tunnels, clear routes, stop proxy |
| `pmux teardown --remove-dns` | Also remove DNS resolver config |

## Per-Run Flow: `pmux run "<command>"`
...
### Route file watching
The proxy polls `routes.json` every 500ms for changes (by mtime). This avoids needing an admin API or IPC mechanism — `pmux run` just writes the file and the proxy picks it up.
...
### Linux
Creates `/etc/systemd/resolved.conf.d/pmux.conf` with `DNS=127.0.0.1` and `Domains=~test`, then restarts `systemd-resolved`.
...
1. **CA cert**: `~/.config/pmux/certs/ca-cert.pem` — 10 year validity
2. **Server cert**: `~/.config/pmux/certs/server-cert.pem` — 1 year validity, wildcard `*.test` + `localhost` SANs
...
Routes are stored in `~/.config/pmux/routes.json`:
...
├── cmd/
│   ├── run.go                 # pmux run — auto-setup, spawn, cleanup
│   ├── proxy.go               # pmux proxy start/run/stop
│   ├── list.go                # pmux list
│   ├── stop.go                # pmux stop <domain>
│   └── teardown.go            # pmux teardown [--remove-dns]
├── internal/
│   ├── proxy/
...
├── go.mod                     # github.com/logscore/pmux
